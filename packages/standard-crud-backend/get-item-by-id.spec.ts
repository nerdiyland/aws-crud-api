...
describe('The `getItemById` method of the items CRUD service', () => {
  ...
  it('should request DynamoDB for the item with the given ID', done => {
    const myService = new ItemsCrud({
      ...
      DynamoDB: {
        // @ts-ignore
        getItem: async (input: GetItemInput) => {
          expect(input.TableName).to.be.equals('dummy');
          expect(input.Key?.Id?.S).to.be.equals('123'); // v3 uses AttributeValue format
          done();
          return {
            Item: {},
          };
  ...
  it('should respond with the item retrieved from the database', () => {
    ...
    expect(myService.getItemById('123'))
      .to.eventually.have.property('CustomerName')
      .equals('dummy');
  ...
  it('should fail if the requested item is not found', () => {
    ...
    expect(myService.getItemById('123')).to.eventually.be.rejectedWith(
      ItemsCrud.ITEM_NOT_FOUND_EXCEPTION
    );
  });
});
